...
weatherRouter.post('/', async (req: Request, res: Response) => {
  const { points } = req.body as { points?: RequestedPoint[] };

  if (!Array.isArray(points) || points.length === 0) {
    res.status(400).json({ error: 'Ingen koordinater oppgitt for værvarsel.' });
    return;
  }

  if (points.length > 12) {
    res.status(400).json({ error: 'For mange værpunkter i én forespørsel (maks 12).' });
    return;
  }
  ...
  res.json({ weather, source: 'MET.no Locationforecast 2.0' });
});
